...
public:
    ...
    {
        std::unique_ptr<Node> newHead = std::make_unique<Node>(newVal);
    ...
    {
        forEach([&](const Node *next) {
            action(*next->Data);
            return true; // means, we should continue to iterate
        });
    }

    std::shared_ptr<T> findFirstIf(std::function<bool(const T &)> predicate)
    {
        std::shared_ptr<T> result;

        forEach([&](const Node *next) mutable {
...
private:
    void forEach(std::function<bool(const Node */*next node*/)> action)
    {
